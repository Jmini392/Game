...
int Core::Init(HWND handle, HINSTANCE hInstance)
{
	m_handle = handle;
	...
	// Root Signature 생성
	Shader::CreateRootSignature(m_pd3dDevice);
...
void Core::Progress()
{
	...
	if (Shader::GetRootSignature())
	{
		m_pd3dCommandList->SetGraphicsRootSignature(Shader::GetRootSignature());
	}
	SceneMgr::Instance()->Render(m_pd3dCommandList);
...
void Core::CreateSwapChain()
{
	...
	dxgiSwapChainDesc.OutputWindow = m_handle;
	...
	hResult = m_pdxgiFactory->MakeWindowAssociation(m_handle, DXGI_MWA_NO_ALT_ENTER);
...
void Core::CreateDirect3DDevice()
{
	...
	// 모든 하드웨어 어댑터에 대해, 특성 레벨 12.0을 지원하는 하드웨어 디바이스를 생성
	...
	// 특성 레벨 12.0을 지원하는 하드웨어 디바이스를 생성할 수 없으면 WARP 디바이스 생성
	...
	// 펜스와 동기화를 위한 이벤트 객체 생성(이벤트 객체의 초기값은 FALSE) / 이벤트가 실행되면(Signal) 이벤트의 값을 자동적으로 FALSE가 되도록 생성
...
void Core::CreateCommandQueueAndList()
{
	...
	hResult = m_pd3dCommandList->Close();
	// 명령 리스트는 생성되면 열린(Open) 상태이므로 닫힌(Close) 상태로 만듦
}
...
void Core::ChangeSwapChainState()
{
	WaitForGpuComplete();
