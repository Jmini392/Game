...
CubeMeshDiffused::CubeMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
    float fWidth, float fHeight, float fDepth, float fRed, float fGreen, float fBlue) : Mesh(pd3dDevice, pd3dCommandList) {
...
GroundMesh::GroundMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight) : Mesh(pd3dDevice, pd3dCommandList)
{
    //직육면체는 꼭지점(정점)이 8개이다. 
    m_nVertices = 4;
    m_nStride = sizeof(Vertex);
